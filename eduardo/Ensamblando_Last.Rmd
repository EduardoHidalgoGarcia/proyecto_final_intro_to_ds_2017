...
#library(melt)
paquetes <- c('readr', 'dplyr', 'ggplot2','reshape2')
...
```{r, warning=FALSE, message = FALSE}
total_Visits$Weekday<-factor(total_Visits$Weekday,levels = c('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'))
total_Visits%>% arrange(desc(Weekday)) %>%
...
```{r, warning=FALSE, message = FALSE}
DepTripType<-total_prep%>%dplyr::mutate(Items=ScanCount)%>%dplyr::filter(DepartmentDescription=="NULL")%>%dplyr::group_by(DepartmentDescription,TripType)%>%dplyr::summarise(TotalItemSold=sum(Items))%>%dplyr::arrange(desc(TotalItemSold))
...
DepTripType%>%dplyr::group_by(NULLDpt,TripTypeNA)%>%dplyr::summarise(NetItemsSold=sum(TotalItemSold))
...
Aux<-train_prep%>%dplyr::group_by(DepartmentDescription,Return)%>%dplyr::summarise(ItemsBoughtReturn=sum(ScanCount),ProductsBoughtReturn=n_distinct(ScanCount))
...
Aux<-train_prep%>%dplyr::group_by(VisitNumber,Weekday,DepartmentDescription)%>%dplyr::summarise(DifferentTripTypes=n_distinct(TripType),ItemsBoughtReturn=sum(ScanCount),ProductsBought=n_distinct(Upc))
...
```{r, warning=FALSE, message = FALSE}
Aux<-train_prep%>%dplyr::group_by(VisitNumber)%>%dplyr::summarise(NetItemsBoughtReturn=sum(ScanCount),NetDistinctUpc=n_distinct(Upc))%>%dplyr::arrange(desc(NetItemsBoughtReturn))
#Aux$Return<-ifelse(Aux$ItemsBoughtReturn<0,'Yes','No')
...
train_clean%>%dplyr::group_by(ReturnVisit, TripClass)%>%dplyr::summarise(NetItemsBoughtReturn=sum(ScanCount),DistinctUpc=n_distinct(Upc))  
  
```
<p>Nos gustaría poder clasificar a cada tipo de visita de acuerdo a la proporcion de compras en cada departamento, por lo que generamos las variables de porcentaje que representan el número de items de cada observacion en el total de su departamento y el número de códigos de barra distintos. Despues de esto transformaremos la base a formato *spread* con base en la columna `VisitNumber`</p>

```{r, warning=FALSE, message = FALSE}
dpt_per_visit<-train_prep%>%
  select(c(TripType,Weekday,VisitNumber,DepartmentDescription,ScanCount))%>%
  #filter(VisitNumber%in%-c("7", "8","132","11","21"))%>%
    dplyr::mutate(return=ifelse(ScanCount>0,"b","r"))%>%
  group_by(TripType,Weekday,VisitNumber,DepartmentDescription,return)%>%
  dplyr::summarise(br_prods=sum(abs(ScanCount))) 

aux<-train_prep%>%
  select(c(TripType,Weekday,VisitNumber,DepartmentDescription,ScanCount))%>%
  #filter(VisitNumber%in%-c("7", "8","132","11","21"))%>%
  group_by(TripType,Weekday,VisitNumber)%>%
  dplyr::summarise(tot_prods=sum(abs(ScanCount))) 

dpt_per_visit2 <- left_join(dpt_per_visit, aux) %>% 
  mutate(share=br_prods/tot_prods) 
  

dpt_per_visit2$DepartmentDescription<-as.character(dpt_per_visit2$DepartmentDescription)
dpt_per_visit2$DepartmentDescAux<-ifelse(dpt_per_visit2$return=='r','RETURN',dpt_per_visit2$DepartmentDescription)
dpt_per_visit2$DepartmentDescription<-as.character(dpt_per_visit2$DepartmentDescription)

dpt_per_visit3<-dpt_per_visit2[,c("TripType","Weekday","VisitNumber","DepartmentDescAux","tot_prods","share")]

dpt_per_visit3$DepartmentDescAux<-as.factor(dpt_per_visit3$DepartmentDescAux)

dpt_per_visit4<-dpt_per_visit3 %>% 
  select("TripType","Weekday","VisitNumber","DepartmentDescAux","tot_prods","share") %>% 
  group_by(VisitNumber,DepartmentDescAux,TripType,Weekday,tot_prods) %>% 
  dplyr::summarise(max_share=max(share)) %>% 
  tidyr::spread(DepartmentDescAux,max_share,fill=0)

write.csv(dpt_per_visit4,"dpt_per_visit.csv",row.names = FALSE)
